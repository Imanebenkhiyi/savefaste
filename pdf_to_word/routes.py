...
def progress():
    ...
    word_filename = f"{session_id}.docx"
    download_link = url_for('pdf_to_word.download_file', filename=word_filename)
    return render_template('pdf_to_word_success.html', download_link=download_link, filename=word_filename)
...
def delete_file(filename):
    print("Received filename:", filename)  # تحقق من الاسم المرسل
    path = os.path.join(CONVERTED_FOLDER, filename)
    print("Full path to delete:", path)    # تحقق من المسار الكامل
