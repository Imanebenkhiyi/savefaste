...
def start_compress():
    if not check_ghostscript():
        return jsonify({"success": False, "message": "Ghostscript is not installed on the server"}), 500
    ...
    try:
        # يمكنك تغيير /ebook إلى /screen أو /prepress حسب الجودة المطلوبة
        command = [
            'gs',
            '-sDEVICE=pdfwrite',
...
def done():
    ...
    # تأكد أن الصفحة موجودة باسم download.html وليس dowland.html
