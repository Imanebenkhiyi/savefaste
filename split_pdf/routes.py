...
def progress():
    ...
    for f in os.listdir(SPLIT_FOLDER):
        if f.startswith(session_id):
            ...
            return render_template('split_success.html', download_link=download_link, filename=f)
...
@split_pdf_bp.route('/delete/<filename>', methods=['DELETE'])
def delete_file(filename):
    path = os.path.join(SPLIT_FOLDER, filename)
