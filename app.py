...
cred = credentials.Certificate('admine.json')
...
from logins.route import auth_bp
from signups.route import sign_bp
from flask import Flask, render_template
import os
from premium_system import premium_bp
...
def create_app():
    ...
    app.register_blueprint(sign_bp, url_prefix='/signup')
    ...
    @app.route('/compressbusines')
    def compressbusines():
       return render_template('compressbusines.html')
   
    @app.route('/Compress PDF Files Before Uploading to WhatsApp and Telegram')
    def CompressPDFFilesBeforeUploadingtoWhatsAppandTelegram():
       return render_template('Compress PDF Files Before Uploading to WhatsApp and Telegram.html')
    
    @app.route("/parametre")
    def parametre():
      return render_template("parametre.html")
    
    @app.route("/Compress PDF Files for Faster Website Uploads")
    def  CompressPDFFilesforFasterWebsiteUploads():
      return render_template("Compress PDF Files for Faster Website Uploads.html")
    
   
    @app.route("/Reduce PDF Size for Email Attachments")
    def  ReducePDFSizeforEmailAttachments():
      return render_template("Reduce PDF Size for Email Attachments.html")
  
    @app.route("/Compress PDF Files Online Without Losing Quality")
    def CompressPDFFilesOnlineWithoutLosingQuality():
      return render_template("Compress PDF Files Online Without Losing Quality.html")

    @app.route("/œœConvert Large PDF Files to Word Online")
    def  ConvertLargePDFFilestoWordOnline():
      return render_template("Convert Large PDF Files to Word Online.html")
    ...
    @app.route("/Convert PDF to Word on Mobile Devices")
    def ConvertPDFtoWordonMobileDevices():
      return render_template("Convert PDF to Word on Mobile Devices.html")
    
    @app.route("/pdf-to-word-no-signup")
    def pdf_to_word_no_signup():
     return render_template("pdftowordsignup.html")
